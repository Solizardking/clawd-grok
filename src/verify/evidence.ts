...
export const VERIFY_ARTIFACT_DIR = ".clawd/verify-artifacts";
...
export function buildReadinessGuidance(profile: VerifyProjectProfile): string[] {
  if (profile.recipe.smokeKind !== "http" || !profile.recipe.startCommand || !profile.recipe.startPort) {
    return [
      "- If you start any process, capture stdout/stderr into a log file under .clawd/verify-artifacts whenever practical.",
    ];
  }
...
export function buildBrowserGuidance(profile: VerifyProjectProfile): string[] {
  if (profile.recipe.smokeKind === "http" && profile.recipe.smokeTarget) {
    return [
      ...
      "- IMPORTANT: All artifact paths MUST be absolute. Use `$(pwd)/${VERIFY_ARTIFACT_DIR}` to construct them. Example: `$(pwd)/.clawd/verify-artifacts/verify-smoke.webm`.",
      "",
